...
export async function downloadMatrixMedia(
  ...
  for (const url of [authUrl, fallbackUrl]) {
    try {
      console.log(`[MatrixMedia] Downloading from ${url.substring(0, 80)}...`);
      const response = await fetch(url, { headers });
      if (response.ok) {
        const data = Buffer.from(await response.arrayBuffer());
        console.log(`[MatrixMedia] Downloaded ${data.length} bytes`);
        return data;
      }
      console.log(`[MatrixMedia] ${url.includes("v1") ? "v1" : "v3"} returned ${response.status}, ${url.includes("v1") ? "trying v3..." : "giving up"}`);
    } catch (err) {
      console.log(`[MatrixMedia] Request failed: ${err}`);
...
export async function downloadAndDecryptMedia(
  ...
  if (encryptionInfo) {
    console.log(`[MatrixMedia] Decrypting E2EE attachment...`);
    const decrypted = await decryptAttachment(data, encryptionInfo);
    console.log(`[MatrixMedia] Decrypted: ${data.length} → ${decrypted.length} bytes`);
    return decrypted;
  }
