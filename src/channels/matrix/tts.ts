...
export async function synthesizeSpeech(
  ...
  console.log(`[MatrixTTS] Synthesizing: ${cleanedText.slice(0, 50)}...`);
  console.log(`[MatrixTTS] Voice: ${voice}, Format: ${format}`);
  ...
  try {
    ...
    console.log(`[MatrixTTS] Synthesized ${audioBuffer.length} bytes`);

    return audioBuffer;
  } catch (err) {
    console.error("[MatrixTTS] Failed to synthesize:", err);
    throw err;
  }
}
...
export async function getAvailableVoices(url?: string): Promise<VoiceInfo[]> {
  ...
  try {
    ...
    console.log(`[MatrixTTS] Found ${voices.length} voices`);
    return voices;
  } catch (err) {
    console.error("[MatrixTTS] Failed to get voices:", err);
