...
export class MatrixVerificationHandler {
  ...
  setupEventHandlers(): void {
    ...
    this.client.on(sdk.ClientEvent.ToDeviceEvent, (event: sdk.MatrixEvent) => {
      ...
      if (type.startsWith("m.key.verification")) {
        console.log(`[MatrixVerification] To-device: ${type} from ${event.getSender()}`, event.getContent());
      }
    });
    ...
    this.client.on(sdk.CryptoEvent.VerificationRequestReceived, (request: sdk.Crypto.VerificationRequest) => {
      console.log(`[MatrixVerification] VerificationRequestReceived: ${request.otherUserId}:${request.otherDeviceId}, phase=${this.phaseName(request.phase)}`);
    ...
    this.client.on(sdk.CryptoEvent.DevicesUpdated, (userIds: string[]) => {
      console.log(`[MatrixVerification] Devices updated: ${userIds.join(", ")}`);
    });

    console.log("[MatrixVerification] Event handlers configured (ready BEFORE startClient())");
  }
  ...
  private handleVerificationRequest(request: sdk.Crypto.VerificationRequest): void {
    ...
    if (existing) {
      ...
      if (existing.request.phase === sdk.Crypto.VerificationPhase.Cancelled ||
        ...
        console.log(`[MatrixVerification] Clearing stale verification: ${otherUserId}:${otherDeviceId}`);
        ...
        console.log(`[MatrixVerification] Replacing stale verification: ${otherUserId}:${otherDeviceId}`);
        this.activeVerifications.delete(key);
      } else {
        console.log(`[MatrixVerification] Already handling: ${otherUserId}:${otherDeviceId}`);
        return;
      }
    }

    console.log(`[MatrixVerification] *** REQUEST from ${otherUserId}:${otherDeviceId} ***`);
    console.log(`[MatrixVerification] Phase: ${this.phaseName(request.phase)}`);
  ...
  private async acceptAndStartSAS(request: sdk.Crypto.VerificationRequest, key: string): Promise<void> {
    try {
      console.log("[MatrixVerification] Accepting verification request...");
      await request.accept();
      console.log(`[MatrixVerification] Accepted, phase is now: ${this.phaseName(request.phase)}`);

      // Check if already Ready (phase might change immediately)
      if (request.phase === sdk.Crypto.VerificationPhase.Ready) {
        console.log("[MatrixVerification] Already Ready, starting SAS immediately...");
        this.startSASVerification(request, key);
        return;
      }
      ...
      const onChange = () => {
        console.log(`[MatrixVerification] Phase changed to: ${this.phaseName(request.phase)}`);
        if (request.phase === sdk.Crypto.VerificationPhase.Ready) {
          console.log("[MatrixVerification] Now in Ready phase, starting SAS...");
      ...
      setTimeout(() => {
        if (request.phase === sdk.Crypto.VerificationPhase.Ready) {
          console.log("[MatrixVerification] Ready detected via timeout, starting SAS...");
          request.off("change" as any, onChange);
          this.startSASVerification(request, key);
        }
      }, 1000);
    } catch (err) {
      console.error("[MatrixVerification] Failed to accept:", err);
  ...
  private async startSASVerification(request: sdk.Crypto.VerificationRequest, key: string): Promise<void> {
    try {
      console.log("[MatrixVerification] Starting SAS verification with m.sas.v1...");
      ...
      if (crypto && request.otherUserId) {
        console.log(`[MatrixVerification] Fetching device keys for ${request.otherUserId}...`);
        await crypto.getUserDeviceInfo([request.otherUserId], true);
        console.log("[MatrixVerification] Device keys fetched");
      ...
      console.log(`[MatrixVerification] Verifier exists: ${!!existingVerifier}`);

      if (existingVerifier) {
        console.log("[MatrixVerification] Verifier already exists, attaching listeners...");
        this.attachVerifierListeners(existingVerifier, request, key);
        return;
      }

      console.log("[MatrixVerification] Calling request.startVerification()...");
      // Start the SAS verification
      const verifier = await request.startVerification("m.sas.v1");

      console.log(`[MatrixVerification] startVerification() returned: ${!!verifier}`);

      if (!verifier) {
        throw new Error("startVerification returned undefined");
      }

      console.log("[MatrixVerification] SAS verifier created");
      ...
      // Attach listeners
      console.log("[MatrixVerification] Attaching verifier listeners...");
      this.attachVerifierListeners(verifier, request, key);

      console.log("[MatrixVerification] Calling verifier.verify()...");
      // Start the verification flow - this sends the accept message
      await verifier.verify();

      console.log("[MatrixVerification] verifier.verify() completed successfully");

    } catch (err) {
      console.error("[MatrixVerification] Error starting SAS:", err);
      this.callbacks.onError?.(err as Error);
    }
  }
  ...
  private attachVerifierListeners(verifier: sdk.Crypto.Verifier, request: sdk.Crypto.VerificationRequest, key: string): void {
    ...
    verifier.on("show_sas" as any, (sas: sdk.Crypto.ShowSasCallbacks) => {
      console.log("[MatrixVerification] *** SHOW SAS (EMOJI) ***");

      if (!sas) {
        console.error("[MatrixVerification] No SAS data received!");
        return;
      }

      const sasData = verifier.getShowSasCallbacks();
      if (!sasData?.sas?.emoji) {
        console.error("[MatrixVerification] No emoji data in SAS!");
        return;
      }

      const emojis = sasData.sas.emoji.map((e: [string, string]) => `${e[0]} ${e[1]}`);
      console.log("[MatrixVerification] Emojis:", emojis.join(" | "));
      console.log("[MatrixVerification] *** COMPARE THESE EMOJIS IN ELEMENT ***");
    ...
    verifier.on("cancel" as any, (err: Error | sdk.MatrixEvent) => {
      console.error("[MatrixVerification] Verification cancelled:", err);
      this.activeVerifications.delete(key);
    ...
    request.on("change" as any, () => {
      ...
      console.log(`[MatrixVerification] Request phase changed: ${this.phaseName(phase)}`);

      if (phase === sdk.Crypto.VerificationPhase.Done) {
        console.log("[MatrixVerification] *** VERIFICATION DONE ***");
        this.activeVerifications.delete(key);
        this.callbacks.onComplete?.();
      } else if (phase === sdk.Crypto.VerificationPhase.Cancelled) {
        console.log("[MatrixVerification] *** VERIFICATION CANCELLED ***");
        this.activeVerifications.delete(key);
  ...
  async confirmVerification(key: string): Promise<void> {
    ...
    if (!stored?.sasCallbacks) {
      console.log("[MatrixVerification] No pending verification to confirm");
      return;
    }

    console.log("[MatrixVerification] Confirming verification (sending MAC)...");
    try {
      await stored.sasCallbacks.confirm();
      console.log("[MatrixVerification] Verification confirmed (MAC sent). Waiting for Done...");
    } catch (err) {
      console.error("[MatrixVerification] Failed to confirm:", err);
      this.callbacks.onError?.(err as Error);
    }
  }
  ...
  async requestVerification(userId: string, deviceId: string): Promise<sdk.Crypto.VerificationRequest> {
    ...
    console.log(`[MatrixVerification] Requesting verification with ${userId}:${deviceId}`);
    ...
    const onReadyOrStarted = () => {
      ...
      if (phase === sdk.Crypto.VerificationPhase.Ready) {
        console.log("[MatrixVerification] Outgoing request ready, starting SAS...");
        this.startSASVerification(request, key);
        request.off("change" as any, onReadyOrStarted);
      } else if (phase === sdk.Crypto.VerificationPhase.Started && request.verifier) {
        console.log("[MatrixVerification] Outgoing request already started, attaching listeners...");
        this.attachVerifierListeners(request.verifier, request, key);
        request.off("change" as any, onReadyOrStarted);
      }
    };
