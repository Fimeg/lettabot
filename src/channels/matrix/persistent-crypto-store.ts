...
export class PersistentCryptoStore {
  ...
  private loadFromDisk(): void {
    try {
      if (existsSync(this.filePath)) {
        ...
        console.log(`[CryptoStore] Loaded from ${this.filePath}`);
      }
    } catch (err) {
      console.warn("[CryptoStore] Failed to load, starting fresh:", err);
      this.data = {};
    }
  }
  ...
  private saveToDisk(): void {
    try {
      ...
      console.error("[CryptoStore] Failed to save:", err);
    }
  }
