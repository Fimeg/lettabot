...
export async function checkAccess(
	...
	// Matrix user IDs are @user:server — direct string comparison, not phone normalization
	if (allowedUsers.includes(userId)) {
		return "allowed";
	}

	// Fall back to upstream pairing store (for runtime-approved users)
	const allowed = await isUserAllowed("matrix", userId, []);
	if (allowed) {
		return "allowed";
	}
