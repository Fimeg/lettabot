...
export async function handleFileMessage(
  ...
  console.log(`[MatrixFile] File from ${sender} in ${roomId}: ${fileInfo.filename} (${fileInfo.mimetype})`);

  if (!fileInfo.mxcUrl) {
    console.warn("[MatrixFile] No URL found in file event");
    return null;
  }
  ...
  try {
    ...
    console.log(`[MatrixFile] Downloaded ${fileData.length} bytes`);
    ...
    console.log(`[MatrixFile] Saved to ${savePath}`);
    ...
    console.error("[MatrixFile] Failed to process file:", err);
    await ctx.sendTyping(roomId, false);
    return null;
  }
}
