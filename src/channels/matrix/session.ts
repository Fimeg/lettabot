...
export class MatrixSessionManager {
	...
	loadSession(): MatrixSession | null {
		try {
			...
			if (!session.userId || !session.accessToken) {
				console.warn("[MatrixSession] Invalid session data, ignoring");
				return null;
			}

			console.log(`[MatrixSession] Loaded session for ${session.userId}`);
			return session;
		} catch (err) {
			console.error("[MatrixSession] Failed to load session:", err);
			return null;
		}
	}
	...
	saveSession(session: MatrixSession): void {
		try {
			...
			console.log(`[MatrixSession] Saved session for ${session.userId}`);
		} catch (err) {
			console.error("[MatrixSession] Failed to save session:", err);
			throw err;
		}
	}
	...
	restoreFromBackup(): MatrixSession | null {
		...
		if (!existsSync(firstBackup)) {
			console.warn("[MatrixSession] No backup available to restore");
			return null;
		}
		...
		try {
			...
			const session = JSON.parse(data) as MatrixSession;
			console.log(`[MatrixSession] Restored from backup for ${session.userId}`);
			return session;
		} catch (err) {
			console.error("[MatrixSession] Failed to restore from backup:", err);
			return null;
		}
	}
	...
	clearSession(): void {
		try {
			...
			console.log("[MatrixSession] Cleared all sessions");
		} catch (err) {
			console.error("[MatrixSession] Failed to clear session:", err);
		}
	}
